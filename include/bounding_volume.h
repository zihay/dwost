...
// Basic math/geometry helpers used by the local BVH scaffold
...
constexpr Float epsilon  = static_cast<Float>(Epsilon);

#ifndef M_PI_2
constexpr Float M_PI_2 = static_cast<Float>(M_PI * 0.5);
#endif

template <typename T>
inline bool inRange(T x, T minVal, T maxVal) {
    return x >= minVal && x <= maxVal;
}

// Generic rotate(): only specialized for DIM=2 and DIM=3 below.
// For other dimensions, just return the input vector unchanged.
template <size_t DIM>
inline Vector<DIM> rotate(const Vector<DIM> &u, const Vector<DIM> & /*v*/,
                          float /*theta*/) {
    return u;
}
...
template <>
inline Vector<2> rotate<2>(const Vector<2> &u, const Vector<2> &v,
                           float theta) {
    ...
    return Vector<2>(cosTheta * u(0) - sinTheta * u(1),
                     sinTheta * u(0) + cosTheta * u(1));
}

// DIM = 3 specialization
template <>
inline Vector<3> rotate<3>(const Vector<3> &u, const Vector<3> &v,
                           float theta) {
    float     cosTheta          = std::cos(theta);
    float     sinTheta          = std::sin(theta);
    Vector<3> w                 = u.cross(v).normalized();
    Vector<3> oneMinusCosThetaW = (1.0f - cosTheta) * w;
...
template <size_t DIM>
struct BoundingBoxSoA {
    std::vector<Vector<DIM>> pMin;
    std::vector<Vector<DIM>> pMax;
    ...
    Vector<DIM> centroid(int i) const {
        return (pMin[i] + pMax[i]) * 0.5f;
    }
};
...
inline void computeOrthonormalBasis(const Vector3 &n, Vector3 &b1, Vector3 &b2) {
    ...
    float       sign = std::copysign(1.0f, n[2]);
...
struct BoundingCone {
    ...
    // constructor
    BoundingCone(const Vector<DIM> &axis_, float halfAngle_, float radius_) : axis(axis_), halfAngle(halfAngle_), radius(radius_) {}
    ...
    bool overlap(const Vector<DIM> &o, const BoundingBox<DIM> &b, float distToBox,
        ...
        if (halfAngle >= M_PI_2 || distToBox < epsilon)
            return true;
        ...
        if (inRange<Float>(M_PI_2, dAxisAngle - halfAngle, dAxisAngle + halfAngle))
            return true;
        ...
        if (l > radius) {
            ...
            return halfAngleSum >= M_PI_2
                       ? true
                       : inRange<Float>(M_PI_2, minAngleRange, maxAngleRange);
        }
...
template <size_t DIM>
struct BoundingConeSoA {
    std::vector<Vector<DIM>> axis;
    std::vector<float>       halfAngle;
